...
app.include_router(
    agent_memory_router,
    prefix="/api/python/agent-memory",
    tags=["agent-memory"]
    # WS4-A: Agent Memory (Redis Iris "Agent Memory" — native equivalent, dual store)
)
app.include_router(
    media_router,
    prefix="/api/python/media",
    tags=["media"],
    dependencies=[Depends(verify_api_key)]
    # Multimodal RAG: cross-modal (CLIP) media ingest + asset status
)
