...
const STORAGE_KEY = 'counsel-theme-mode';

/**
 * Custom hook for managing Counsel theme state (dark/light mode)
...
export function useZenTheme(defaultMode: ZenThemeMode = 'dark'): ZenThemeContext {
  ...
  useEffect(() => {
    try {
      ...
      // If no stored value, keep the caller-provided default (light in counsel)
    } catch (e) {
