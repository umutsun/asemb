 ...
 * Editorial law-review chat: serif display, single viridian accent,
 * footnote-style sources, with its own dark/light mode
 * (container-scoped theme tokens; light is warm paper).
 */
export default function ChatInterface() {
  ...
  // Theme state (editorial paper look defaults to light)
  const { isDark, toggle: toggleTheme } = useZenTheme('light');
    ...
    Promise.all([
      ...
      .then(async ([chatbotRes, llmRes, ragRes, promptsRes]) => {
        ...
        const config: ZenChatbotSettings = {
          ...
          // editorial markdown renderer is used instead of a schema renderer)
        ...
        if (category === 'chatbot') {
          ...
          if (chatbotRes.ok) {
            ...
            setChatbotSettings(prev => ({
              ...
              // schema id must take effect immediately (editorial markdown branch).
              responseSchemaId: chatbotData.responseSchemaId
            }));
    ...
    fetch('/api/v2/chat/pdf-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Counsel] PDF settings loaded:', { masterToggle, endpointEnabled: data.enabled });
        }
      })
      .catch(err => {
        console.error('[Counsel] Failed to fetch PDF settings:', err);
      });
  ...
  useEffect(() => {
    ...
    if (!masterInputToggle && !masterOutputToggle) {
      ...
      console.log('[Counsel] Voice features disabled by master toggles');
      return;
    }
    ...
    fetch('/api/v2/chat/voice-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Counsel] Voice settings loaded (with master toggles):', {
      ...
      .catch(err => {
        console.error('[Counsel] Failed to fetch voice settings:', err);
      });
  ...
  return (
    <ProtectedRoute>
      <div
        className={`counsel-container ${isDark ? 'dark' : 'light'}`}
        data-theme="counsel"
