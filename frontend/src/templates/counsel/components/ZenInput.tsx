...
const GHOST_BTN =
  'rounded-md p-2 text-[var(--counsel-muted)] transition-colors hover:bg-[var(--counsel-hover)] hover:text-[var(--counsel-ink)] disabled:opacity-40';

/**
 * Counsel Input Component
 * Sticky bottom composer: surface container with hairline border and sharp
 * editorial corners, serif autosize textarea, ghost secondary icons and a
 * solid accent uppercase send button.
 */
export const ZenInput: React.FC<ZenInputProps> = ({
  ...
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-[var(--counsel-bg)] via-[var(--counsel-bg)] to-transparent pb-5 pt-8">
      ...
      <div className="mx-auto w-full max-w-2xl px-4">
        ...
        {pdfFile && (
          <div className="mb-2">
            <div className="inline-flex items-center gap-2 rounded border border-[var(--counsel-hairline)] bg-[var(--counsel-surface)] px-3 py-1.5 text-[var(--counsel-ink)]">
              <FileText className="h-4 w-4 text-[var(--counsel-muted)]" />
              <span className="max-w-[200px] truncate text-sm">
                {pdfFile.name}
              </span>
              <span className="text-xs text-[var(--counsel-muted)]">
                ({formatFileSize(pdfFile.size)})
              </span>
              <button
                type="button"
                onClick={handleRemovePdf}
                className="rounded p-0.5 text-[var(--counsel-muted)] transition-colors hover:text-[var(--counsel-ink)]"
                title="Remove PDF"
              >
        ...
        {/* Editorial composer: surface, hairline border, sharp 4px corners (counsel.css) */}
        <div className="counsel-composer relative flex items-end gap-1">
          ...
          {pdfSettings?.enabled && (
            <button
              ...
              className={`${GHOST_BTN} ${pdfFile ? 'text-[var(--counsel-accent)]' : ''}`}
          ...
          <button
            ...
            className={`${GHOST_BTN} ${showSlashAutocomplete ? 'text-[var(--counsel-accent)]' : ''}`}
            title="Commands"
          >
          ...
          {voiceSettings?.enableVoiceInput && (
            <button
              ...
              className={`${GHOST_BTN} ${
                isRecording
                  ? 'animate-pulse text-[var(--counsel-danger)]'
                  : isTranscribing
          ...
          <textarea
            ...
            className="max-h-40 flex-1 resize-none border-none bg-transparent px-2 py-1.5 text-[15px] leading-relaxed text-[var(--counsel-ink)] placeholder:text-[var(--counsel-muted)] focus:outline-none focus:ring-0 disabled:opacity-50"
          />

          {/* Solid accent send button: localized uppercase label, sharp 3px corners */}
          <button
            type="button"
            onClick={handleSend}
            disabled={(!value.trim() && !pdfFile) || isLoading || isRecording}
            className="counsel-send my-0.5 flex items-center justify-center"
            aria-label={t('chatInput.send')}
          >
            {isLoading ? (
              /* Spinner inherits the on-accent color so it stays legible per mode */
