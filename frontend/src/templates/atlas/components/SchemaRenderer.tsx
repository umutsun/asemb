  ...
  return (
    <div className={`schema-response ${className}`}>
      ...
      {/* Fallback: If no sections rendered, show raw content
          (atlas-prose keys colors to the container's own theme class) */}
...
const SectionRenderer: React.FC<SectionRendererProps> = ({ section, content, messageId, citationDisplayMap }) => {
  ...
  switch (section.style) {
    case 'heading':
      return (
        <div className="mb-4">
          <h3 className="mb-1 text-sm font-semibold text-[var(--atlas-ink)]">
            {section.label}
          </h3>
          <p className="text-base font-medium text-[var(--atlas-ink)]">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag, idx) => (
              <span
                key={idx}
                className="inline-flex items-center rounded border border-[var(--atlas-border)] px-2.5 py-0.5 text-xs text-[var(--atlas-muted)]"
              >
                {tag}
              </span>
            ))}
    ...
    case 'citation':
      ...
      return (
        <div className="mb-4">
          <h4 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[var(--atlas-muted)]">
            {section.label}
          </h4>
          {/* Quiet citation list - muted start border */}
          <div className="space-y-2 border-s-2 border-[var(--atlas-border)] ps-3 text-xs">
            {citations.map((cite, idx) => (
              <div key={idx} className="flex items-start gap-2 py-1">
                <span className="min-w-[24px] flex-shrink-0 font-mono text-[var(--atlas-muted)]">
                  [{idx + 1}]
                </span>
                <span className="italic leading-relaxed text-[var(--atlas-muted)]">
    ...
    case 'list':
      ...
      return (
        <div className="mb-4">
          ...
          <ul className="list-disc list-outside ms-4 space-y-1 text-sm text-[var(--atlas-ink)]">
            {items.map((item, idx) => (
            ...
            if (citationNum) {
              ...
              const scrollToSource = (e: React.SyntheticEvent) => {
                ...
                if (!sourceEl) {
                  ...
                  const sourceItems = document.querySelectorAll('.atlas-source-item');
                ...
                if (sourceEl) {
                  ...
                  const row = (sourceEl.closest('.atlas-source-item') as HTMLElement) || sourceEl;
                  ...
                  setTimeout(() => { row.classList.remove('atlas-cite-flash'); }, 1700);
                }
              };
              return (
                ...
                <bdi key={`cite-${idx}`} dir="ltr">
                  <sup
                    className="atlas-cite atlas-cite-clickable"
                    role="button"
                    tabIndex={0}
      ...
      return (
        <div className="mb-4">
          {/* Hide section label - show content directly. Colors come from the
              container-scoped tokens (atlas-prose), not global dark: variants. */}
          {/* Headings, strong, and ordered-list number tiles come from the
              .atlas-prose CSS (atlas.css); only ul/blockquote need utilities */}
          <div className="atlas-prose max-w-none
                          [&_ul]:my-2 [&_ul]:ms-4 [&_ul]:list-disc
                          [&_blockquote]:border-s-2 [&_blockquote]:border-[var(--atlas-border)] [&_blockquote]:ps-4 [&_blockquote]:italic [&_blockquote]:text-[var(--atlas-muted)]">
            <ReactMarkdown
              ...
              components={{
                ...
                // Ordered lists render bare (counter-driven number tiles come
                // from .atlas-prose ol CSS). CSS counters ignore the <ol start>
                // attribute, so honor markdown's `start` prop by seeding the
                // counter. Cast props: react-markdown types omit `start`.
                ...
                // The wrapper div keeps inline flow intact inside the flex row
                // an ordered-list item becomes (number tile + body).
                li: ({ children }) => (
                  <li>
