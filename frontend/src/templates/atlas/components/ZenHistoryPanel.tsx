...
/**
 * ZenHistoryPanel - popover card above the input (surface bg, border, 12px radius, large shadow)
 */
export const ZenHistoryPanel: React.FC<ZenHistoryPanelProps> = ({
  ...
  return (
    <div ref={panelRef} className="atlas-panel atlas-msg-in">
      {/* Search */}
      <div className="atlas-panel-search">
      ...
      {/* Conversations List */}
      <ScrollArea className="atlas-panel-list">
        {isLoading ? (
          <div className="py-1">
            <div className="atlas-panel-skeleton" />
            <div className="atlas-panel-skeleton" />
            <div className="atlas-panel-skeleton" />
          </div>
          ...
          groupedConversations.map(([group, convs]) => (
            <div key={group}>
              <div className="atlas-panel-label">{group}</div>
              {convs.map(conv => (
                <div
                  key={conv.id}
                  onClick={() => {
                  ...
                  className={`atlas-panel-row ${
                    conv.id === currentConversationId ? 'active' : ''
                  }`}
                >
                  <span className="atlas-history-item-title">
                    {conv.title || 'Untitled conversation'}
                  </span>
                  <span className="atlas-history-item-time">
                  ...
                  {confirmDeleteId === conv.id ? (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="atlas-history-confirm-yes"
                      >
                        Delete
                      </button>
                      <button
                        ...
                        className="atlas-history-confirm-no"
                    ...
                    <button
                      type="button"
                      onClick={(e) => handleDeleteClick(e, conv.id)}
                      className="atlas-history-item-delete"
                      title="Delete"
                    >
