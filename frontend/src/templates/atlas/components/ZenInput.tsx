...
import { Paperclip, X, FileText, Mic, Square, Loader2, Command, ArrowUp } from 'lucide-react';
...
const GHOST_BTN =
  'rounded-md p-2 text-[var(--atlas-muted)] transition-colors hover:bg-[var(--atlas-surface-2)] hover:text-[var(--atlas-ink)] disabled:opacity-40';

/**
 * Atlas Input Component
 * Sticky bottom composer: floating surface card with soft shadow and 16px
 * corners, sans autosize textarea, ghost secondary icons and a square accent
...
export const ZenInput: React.FC<ZenInputProps> = ({
  ...
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-[var(--atlas-bg)] via-[var(--atlas-bg)] to-transparent pb-5 pt-8">
      ...
      <div className="mx-auto w-full max-w-[700px] px-4">
        ...
        {pdfFile && (
          <div className="mb-2">
            <div className="inline-flex items-center gap-2 rounded-lg border border-[var(--atlas-border)] bg-[var(--atlas-surface)] px-3 py-1.5 text-[var(--atlas-ink)]">
              ...
              <button
                ...
                className="rounded p-0.5 text-[var(--atlas-muted)] transition-colors hover:text-[var(--atlas-ink)]"
                title="Remove PDF"
              >
        ...
        {/* Floating composer card: surface, border, 16px corners, large shadow (atlas.css) */}
        <div className="atlas-composer relative flex items-end gap-1">
          ...
          {pdfSettings?.enabled && (
            <button
              ...
              className={`${GHOST_BTN} ${pdfFile ? 'text-[var(--atlas-accent)]' : ''}`}
          ...
          <button
            ...
            className={`${GHOST_BTN} ${showSlashAutocomplete ? 'text-[var(--atlas-accent)]' : ''}`}
            title="Commands"
          >
          ...
          {voiceSettings?.enableVoiceInput && (
            <button
              ...
              className={`${GHOST_BTN} ${
                isRecording
                  ? 'animate-pulse text-[var(--atlas-danger)]'
                  : isTranscribing
          ...
          <textarea
            ...
            className="max-h-40 flex-1 resize-none border-none bg-transparent px-2 py-1.5 text-[15px] leading-relaxed text-[var(--atlas-ink)] placeholder:text-[var(--atlas-muted)] focus:outline-none focus:ring-0 disabled:opacity-50"
          />

          {/* Square accent send button: up-arrow icon, localized aria-label */}
          <button
            type="button"
            onClick={handleSend}
        ...
        {/* Under-composer hint line (existing i18n key, shown on wider screens) */}
        <div className="atlas-composer-hint hidden sm:block">
