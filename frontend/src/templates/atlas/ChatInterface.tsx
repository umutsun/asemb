 ...
 * Product-grade chat: layered surfaces, soft shadows, ultramarine accent,
 * icon source cards and pill follow-ups, with its own dark/light mode
 * (container-scoped theme tokens; defaults to light).
 */
export default function ChatInterface() {
  ...
  // Theme state (Atlas defaults to light)
    ...
    fetch('/api/v2/chat/pdf-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Atlas] PDF settings loaded:', { masterToggle, endpointEnabled: data.enabled });
        }
      })
      .catch(err => {
        console.error('[Atlas] Failed to fetch PDF settings:', err);
      });
  ...
  useEffect(() => {
    ...
    if (!masterInputToggle && !masterOutputToggle) {
      ...
      console.log('[Atlas] Voice features disabled by master toggles');
      return;
    }
    ...
    fetch('/api/v2/chat/voice-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Atlas] Voice settings loaded (with master toggles):', {
      ...
      .catch(err => {
        console.error('[Atlas] Failed to fetch voice settings:', err);
      });
  ...
  return (
    <ProtectedRoute>
      <div
        className={`atlas-container ${isDark ? 'dark' : 'light'}`}
        data-theme="atlas"
      ...
      >
        ...
        {/* Main Chat Area - single centered reading column (mock .col: 700px, 28px gap) */}
        <div className="relative z-10 mx-auto w-full max-w-[700px] overflow-hidden px-4">
