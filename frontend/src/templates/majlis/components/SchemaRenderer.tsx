  ...
  return (
    <div className={`schema-response ${className}`}>
      ...
      {/* Fallback: If no sections rendered, show raw content
          (majlis-prose keys colors to the container's own theme class) */}
      {visibleSections.length === 0 && (
        <div className="majlis-prose max-w-none">
...
const SectionRenderer: React.FC<SectionRendererProps> = ({ section, content, messageId, citationDisplayMap }) => {
  ...
  switch (section.style) {
    case 'heading':
      return (
        <div className="mb-4">
          <h3 className="mb-1 text-sm font-semibold text-[var(--majlis-ink)]">
            {section.label}
          </h3>
          <p className="text-base font-medium text-[var(--majlis-ink)]">
    ...
    case 'tags':
      ...
      return (
        <div className="mb-4">
          {/* Hide label for keywords - show tags directly as sharp small-caps hairline chips */}
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag, idx) => (
              <span
                key={idx}
                className="inline-flex items-center rounded-sm border border-[var(--majlis-hairline)] px-2.5 py-0.5 text-[10.5px] uppercase tracking-[0.08em] text-[var(--majlis-muted)]"
              >
                {tag}
              </span>
            ))}
    ...
    case 'citation':
      ...
      return (
        <div className="mb-4">
          <h4 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[var(--majlis-muted)]">
            {section.label}
          </h4>
          {/* Quiet citation list - hairline start border */}
          <div className="space-y-2 border-s-2 border-[var(--majlis-hairline)] ps-3 text-xs">
            {citations.map((cite, idx) => (
              <div key={idx} className="flex items-start gap-2 py-1">
                <span className="min-w-[24px] flex-shrink-0 font-mono text-[var(--majlis-muted)]">
                  [{idx + 1}]
                </span>
                <span className="italic leading-relaxed text-[var(--majlis-muted)]">
    ...
    case 'list':
      ...
      return (
        <div className="mb-4">
          ...
          <ul className="list-disc list-outside ms-4 space-y-1 text-sm text-[var(--majlis-ink)]">
            {items.map((item, idx) => (
            ...
            if (citationNum) {
              ...
              const scrollToSource = (e: React.SyntheticEvent) => {
                ...
                if (!sourceEl) {
                  ...
                  const sourceItems = document.querySelectorAll('.majlis-source-item');
                ...
                if (sourceEl) {
                  ...
                  const row = (sourceEl.closest('.majlis-source-item') as HTMLElement) || sourceEl;
                  ...
                  setTimeout(() => { row.classList.remove('majlis-cite-flash'); }, 1700);
                }
              };
              return (
                ...
                <bdi key={`cite-${idx}`} dir="ltr">
                  <sup
                    className="majlis-cite cursor-pointer no-underline hover:underline underline-offset-2"
                    role="button"
                    tabIndex={0}
      ...
      return (
        <div className="mb-4">
          {/* Hide section label - show content directly. Colors come from the
              container-scoped tokens (majlis-prose), not global dark: variants.
              Ordered lists intentionally have NO utility overrides: majlis.css
              renders them with mono decimal-leading-zero brass numerals. */}
          <div className="majlis-prose max-w-none
                          ...
                          [&_ul>li]:my-1 [&_ul>li]:ps-1
                          [&_strong]:font-semibold [&_strong]:text-[var(--majlis-ink)]
                          [&_blockquote]:border-s-2 [&_blockquote]:border-[var(--majlis-hairline)] [&_blockquote]:ps-4 [&_blockquote]:italic [&_blockquote]:text-[var(--majlis-muted)]">
            <ReactMarkdown
