...
export const SlashCommandAutocomplete: React.FC<SlashCommandAutocompleteProps> = ({
  ...
  return (
    <div className="majlis-panel majlis-msg-in">
      <div className="majlis-panel-list">
        {commands.map((cmd, idx) => (
          <div key={cmd.id}>
            <div
              onClick={() => handleCommandClick(cmd)}
              className={`majlis-panel-row ${idx === selectedIndex ? 'selected' : ''}`}
            >
              <span className="majlis-slash-trigger">{cmd.trigger}</span>
              <span className="text-sm text-[var(--majlis-ink)]">{cmd.label}</span>
              {cmd.hasSubmenu && (
                <ChevronRight
                  className={`ms-auto h-4 w-4 text-[var(--majlis-muted)] transition-transform ${
              ...
              <div>
                {cmd.submenuItems.map((item, subIdx) => (
                  <div
                    ...
                    className={`majlis-panel-row majlis-slash-submenu-row text-sm ${
