...
/**
 * Majlis Chat Interface
 * Ink-and-brass legal chat: small-caps labels, docket-style source ledger
 * with dotted leaders, serif questions, with its own dark/light mode
 * (container-scoped theme tokens; the design is dark-first).
 */
export default function ChatInterface() {
  ...
  // Theme state (ink-and-brass is dark-first: defaults to dark)
    ...
    fetch('/api/v2/chat/pdf-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Majlis] PDF settings loaded:', { masterToggle, endpointEnabled: data.enabled });
        }
      })
      .catch(err => {
        console.error('[Majlis] Failed to fetch PDF settings:', err);
      });
  ...
  useEffect(() => {
    ...
    if (!masterInputToggle && !masterOutputToggle) {
      ...
      console.log('[Majlis] Voice features disabled by master toggles');
      return;
    }
    ...
    fetch('/api/v2/chat/voice-settings', {
      ...
      .then(data => {
        if (!data.error) {
          ...
          console.log('[Majlis] Voice settings loaded (with master toggles):', {
      ...
      .catch(err => {
        console.error('[Majlis] Failed to fetch voice settings:', err);
      });
  ...
  return (
    <ProtectedRoute>
      <div
        className={`majlis-container ${isDark ? 'dark' : 'light'}`}
        data-theme="majlis"
