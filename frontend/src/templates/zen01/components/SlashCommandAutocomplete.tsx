...
export const SlashCommandAutocomplete: React.FC<SlashCommandAutocompleteProps> = ({
  ...
  return (
    <div className="zen01-panel zen01-msg-in">
      <div className="zen01-panel-list">
        {commands.map((cmd, idx) => (
          <div key={cmd.id}>
            <div
              onClick={() => handleCommandClick(cmd)}
              className={`zen01-panel-row ${idx === selectedIndex ? 'selected' : ''}`}
            >
              <span className="zen01-slash-trigger">{cmd.trigger}</span>
              <span className="text-sm text-[var(--zen-ink)]">{cmd.label}</span>
              {cmd.hasSubmenu && (
                <ChevronRight
                  className={`ms-auto h-4 w-4 text-[var(--zen-muted)] transition-transform ${
              ...
              <div>
                {cmd.submenuItems.map((item, subIdx) => (
                  <div
                    ...
                    className={`zen01-panel-row zen01-slash-submenu-row text-sm ${
